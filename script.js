...
function init() {
    ...
    btnReturn.addEventListener('click', onReturnClick);
    initThreeJS();
    setupVoiceMode();
}

// Restart button is shared between modes
...
// Return button (champion overlay) is shared between modes
...
/* ============================================================
   2D VOICE MODE  (You = X by voice/click, AI = O)
   ============================================================ */

// Mode + 2D state
let gameMode = '3d'; // '3d' | '2d'
let board2D = [["-", "-", "-"], ["-", "-", "-"], ["-", "-", "-"]];
...
const instructions3D = document.getElementById('instructions-3d');
...
// Wire up mode toggle, difficulty selector, voice button, and build the board
...
function switchMode(mode) {
    ...
    // Hide champion overlay and clear shared score display
    championOverlay.classList.remove('show');
    scoreX.innerHTML = '';
    scoreO.innerHTML = '';
    ...
    if (mode === '2d') {
        view3D.hidden = true;
        view2D.hidden = false;
        instructions3D.hidden = true;
        voiceControls.hidden = false;
        score2D = { X: 0, O: 0 };
        resetBoard2D();
        speak("Two D voice mode. Say a cell to play.");
    } else {
        stopListening();
        cancelSpeech();
        view3D.hidden = false;
        view2D.hidden = true;
        instructions3D.hidden = false;
...
function playerMove2D(row, col) {
    if (gameMode !== '2d' || !gameActive2D || aiThinking) return false;
    if (board2D[row][col] !== "-") {
...
function endRound2D(result) {
    ...
    // Champion check (best of 3)
    if (score2D[w] === 3) {
        setTimeout(() => {
            championText.textContent = w === 'X' ? 'You are the Champion!' : 'AI is the Champion';
            championOverlay.classList.add('show');
            speak(w === 'X'
                ? "Congratulations! You are the champion!"
                : "I am the champion. Better luck next time!");
        }, 1200);
    }
}

function addScoreMark2D(player) {
    const container = player === 'X' ? scoreX : scoreO;
    const mark = document.createElement('div');
    mark.className = `score-mark player-${player.toLowerCase()}`;
    mark.textContent = player;
    container.appendChild(mark);
}

// Clear the board for a new round (keeps scores)
...
function restartRound2D() {
    resetBoard2D();
}

function resetGame2D() {
    score2D = { X: 0, O: 0 };
    scoreX.innerHTML = '';
    scoreO.innerHTML = '';
    championOverlay.classList.remove('show');
...
function handleTranscript(transcript) {
    ...
    if (tokens.some(t => ['restart', 'reset', 'again'].includes(t)) ||
        text.includes('new game') || text.includes('play again')) {
        restartRound2D();
        speak("New round. Your turn.");
        return true;
    }
...
function parseCell(tokens) {
    ...
    // Fill remaining coordinate(s) with the middle/center value
